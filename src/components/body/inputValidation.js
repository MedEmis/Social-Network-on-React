...
export default class BoundedInput extends React.Component {
	constructor(props) {
		super(props);
	...
	results() {
		...
		if (this.props.max === undefined && this.props.recommendedMax === undefined) {
			...
			var percentage = this.max !== 'infinite' ? this.current / this.max : this.current / this.recommendedMax;
		}
	...
	render() {
		...
		const { type, rows, fontFamily, fontSize, fontWeight, label, required, id, imgSrs } = this.props;
		const cloneStyles = {
			position: 'absolute',
			whiteSpace: 'nowrap',
		...
		return (
			<div className={"input-field col s12 bounded-input bounded-input--" + status}>
				<i className="material-icons prefix"><img src={imgSrs} alt="icon" /></i>
				...
				{type === 'textarea' ? (
					<textarea rows={rows} onChange={this.handleTyping} value={value} />
						...
						<input
							...
							value={value}
							className="reginput validate"
							required={required}
							onInput={(event) => this.props.infoPreviev(event)}
						/>
					)}
...
// 				<h4>Recommended Min & Recommended Max</h4>
// 				<div className="field">
...
// 				</div>
// 				<h4>Strict Min & Strict Max</h4>
// 				<div className="field">
// 					<label>Max 30</label>
// 					<BoundedInput type="text" max="30" />
// 				</div>
// 				<div className="field">
// 					<label>Min 15</label>
