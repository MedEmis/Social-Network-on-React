...
let store = {
	...
	addNewReply(userId, id, nikName, event) {
	...
	addNewNestedReply(userId, initialUser, initialPost, id, nikName, event) {
		let textArea = event.target.offsetParent.lastElementChild.childNodes[0]
		let time = new Date().toLocaleTimeString().slice(0, -3)
		let date = new Date().toLocaleDateString()
		let post = {
	...
	subscribe(observer) { this.reRenderApp = observer },
	updatePostText(inputValue) {
